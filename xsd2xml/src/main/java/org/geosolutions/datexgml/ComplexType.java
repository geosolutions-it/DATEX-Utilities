...
public class ComplexType {
    ...
    final List<ComplexType> extensions = new ArrayList<>();
    ...
    public void buildResultNodes() {
        ...
        // add extensions nodes
        extensions.forEach(
                extension -> {
                    GmlConverter.xpath(extension.getNode(), "child::*")
                            .collect(Collectors.toList())
                            .stream()
                            .filter(c -> c.getNodeName().equals("annotation"))
                            .forEach(
                                    extChild -> {
                                        Node cloned = extChild.cloneNode(true);
                                        complexTypeNode.appendChild(cloned);
                                    });
                });
        ...
        // if it extends another type, build internal GML extension
        if (!extendsAnother) {
    ...
    protected void buildPropertyTypeNode(String name) {
        // complexType
        Element ptNode =
                node.getOwnerDocument().createElementNS(GmlConverter.XS_NS, "xs:complexType");
        ptNode.setAttribute("name", name + PROPERTY_TYPE_SUFIX);
        complexPropertyTypeNode = ptNode;
        // sequence
        Element sequence =
                node.getOwnerDocument().createElementNS(GmlConverter.XS_NS, "xs:sequence");
        ...
        Element attributeGroup =
                node.getOwnerDocument().createElementNS(GmlConverter.XS_NS, "xs:attributeGroup");
    ...
    protected void buildExtendedContent() {
        ...
        Element complexContentEl =
                node.getOwnerDocument().createElementNS(GmlConverter.XS_NS, "xs:complexContent");
        complexTypeNode.appendChild(complexContentEl);
        Element extensionEl =
                node.getOwnerDocument().createElementNS(GmlConverter.XS_NS, "xs:extension");
        extensionEl.setAttribute("base", "gml:AbstractFeatureType");
        complexContentEl.appendChild(extensionEl);
        // append original childes
        childs.stream()
                .filter(c -> !c.getNodeName().equals("annotation"))
                .forEach(
                        c -> {
